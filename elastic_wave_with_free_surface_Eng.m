...
dt =  0.0005;          % step length of time,unit s
tt = -0.04:dt:0.04;    % the basic frequency is 60HZ
fm = 60;
A = 1;
...
wave=[0 0 wave];
T = 2;              % propagation time of the wave, unit s
wave(round(T/dt))=0;    % zero fill the wavelet

load 'model_multiples.mat'
% Vv=importdata('V1.mat');      % velocity of P wave,m/s
% Vs=importdata('V2.mat');      % velocity of S wave,m/s
V2(:) = 0;
% den=importdata('density.mat');     % medium density£¬kg/m^3
Vv = V1; Vs = V2; den = density;

[Nz,Nx] = size(Vv);
pml = 50;                              % the grid number of pml
m = Nx + 2*pml;
n = Nz + 2*pml;
dz = 5;                               %m
dx = 5;
% d=2000*ones(n,m);                    % medium density,kg/m^3

V=zeros(n,m);                        % medium velocity of P wave,m/s,zero fill
...
clear Vv;
Vmax=max(max(V));         % max velocity of P wave

VS=zeros(n,m);                        % medium velocity of S wave,m/s,zero fill
...
d=zeros(n,m);                        % medium density,kg/m^3,zero fill
...
% information of sources and receivers
z0 = pml + 180;                       % z position of source
shot0 = pml+150;
shotnum = length(shot0);  % number of source
% shot0=round(m/2);
rz0 = pml + 1;
rx0 = pml+1:pml+Nx;
nt = round(T/dt);
...
for shot = 1%:shotnum
    ...
    disp(['begin the source No.' num2str(shot) 'forward modeling, the total number is' num2str(shotnum) '']);
...
%          Vz(z0,x0) = Vz(z0,x0) + wave(it);
         ...
         % record the real-time wave feild at present
         record1(it,:,shot) =Vz(rz0,rx0);
         record2(it,:,shot)=Vx(rz0,rx0)
         
         %¡­¡­¡­¡­¡­¡­the wave feild display program¡­¡­¡­¡­¡­¡­¡­¡­
          subplot(1,2,1);
          imagesc(Pz*10e9);
          shading interp;
          axis square;
